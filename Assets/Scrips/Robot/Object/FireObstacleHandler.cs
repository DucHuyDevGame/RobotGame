using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class FireObstacleHandler : MonoBehaviour, IObstacleHandler
{
    private RobotController robot;
    private DataController data;
    private GameObject fireCheck;
    public void Init(RobotController robot, DataController data, GameObject fireCheck)
    {
        this.robot = robot;
        this.data = data;
        this.fireCheck = fireCheck;
    }

    public void HandleObstacle(Vector2 origin, Vector2 direction, float distance, LayerMask layer)
    {
        RaycastHit2D hit = Physics2D.Raycast(origin, direction, distance, layer);
        Debug.DrawRay(origin, direction * distance, Color.green);
        if (hit.collider == null)
            return;
        WeaponsData weapons = data.ReloadWeapon();
        if (weapons.sensorTypeData.sensorType != SensorsType.HeatSensor)
        {
            robot.runRobot = false;
            DialogManager.Instance.ShowDialog(DialogIndex.DiedDialog);
            Debug.DrawRay(origin, direction * distance, Color.red);
            return;
        }
        else
        {
            robot.runRobot = false;
            if (weapons.manipulatorData.manipulatorType == ManipulatorType.FireExtinguisherSpray)
                StartCoroutine(WaitTimeRun());
            Debug.DrawRay(origin, direction * distance, Color.blue);
        }
    }
    IEnumerator WaitTimeRun()
    {
        yield return new WaitForSeconds(0.2f);
        fireCheck.SetActive(false);
        robot.runRobot = true;
    }
}
